...
type (
	dissector struct {
		...
		dissectedCallback func(*Input)   // called on each analyzed result
		closedCallback    func()         // called when .close() is called and no more calls to analyzedCallback will be made
		dissectChannel    chan *Input    // holds unanalyzed data
		dissectWg         sync.WaitGroup // wait for analysis to finish
	}
)

//newdissector creates a new collector for gathering data
func newDissector(connLimit int64, db *database.DB, conf *config.Config, dissectedCallback func(*Input), closedCallback func()) *dissector {
	return &dissector{
		connLimit:         connLimit,
		db:                db,
		conf:              conf,
		...
		dissectChannel:    make(chan *Input),
	}
}

//collect sends a chunk of data to be analyzed
func (d *dissector) collect(entry *Input) {
